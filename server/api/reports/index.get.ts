...
defineRouteMeta({
  openAPI: {
    tags: ['reports'],
    summary: '週の日報一覧取得',
    description: '全ロール対象。新人は自分、メンター・OJT は担当新人、管理者は全員の日報を取得（RLS で強制）。メンター・OJT・管理者は userId 必須、新人は省略。',
    parameters: [
      { in: 'query', name: 'weekStart', required: true, schema: { type: 'string', format: 'date' }, description: '取得対象週の月曜日（YYYY-MM-DD）' },
      { in: 'query', name: 'userId', required: false, schema: { type: 'string', format: 'uuid' }, description: '対象ユーザーID。新人は省略（自分のみ）' }
    ],
    responses: {
      200: { description: '日報一覧', content: { 'application/json': { example: [{ id: 'uuid', user_id: 'uuid', date: '2026-05-19', check_in: '09:00:00', check_out: '18:00:00', content: 'やったことの本文', mood: 4, created_at: '2026-05-19T09:00:00Z', updated_at: '2026-05-19T09:00:00Z' }] } } },
