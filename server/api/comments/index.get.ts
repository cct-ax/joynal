...
defineRouteMeta({
  openAPI: {
    tags: ['comments'],
    summary: '週次コメント取得',
    description: '全ロール対象。新人は自分宛、メンター・OJT は担当新人宛、管理者は全員のコメントを取得（RLS で強制）。commenter に投稿者の name / role を結合して返す。',
    parameters: [
      { in: 'query', name: 'weekStart', required: true, schema: { type: 'string', format: 'date' }, description: '対象週の月曜日（YYYY-MM-DD）' },
      { in: 'query', name: 'traineeId', required: true, schema: { type: 'string', format: 'uuid' }, description: '対象新人のユーザーID' }
    ],
    responses: {
      200: { description: '週次コメント一覧', content: { 'application/json': { example: [{ id: 'uuid', week_start: '2026-05-19', trainee_id: 'uuid', commenter_id: 'uuid', commenter: { name: '田中 太郎', role: 'mentor' }, content: '今週もよく頑張りました。', created_at: '2026-05-23T18:00:00Z', updated_at: '2026-05-23T18:00:00Z' }] } } },
