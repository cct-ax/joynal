...
| `app/types/database.types.ts` | Supabase 自動生成ファイル — **編集禁止** |
| `app/types/schemas.ts` | フォームバリデーション用 Zod スキーマ（型は `z.output<>` で導出） |
...
- **型の置き場所**: app・server 両方で使う型は `shared/types/`、フォームの Zod スキーマは `app/types/schemas.ts`、コンポーネント内にインラインで型定義しない
- **データ取得**: `onMounted + $fetch` より `useFetch` を優先、`$fetch` は必ず `try/catch` + トースト通知でラップ
