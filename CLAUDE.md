...
pnpm test:e2e      # Playwright E2E（要ローカル Supabase: supabase start → db reset）
...
| `app/components/` | ドメイン別フォルダ: `report/`（日報UI）・`admin/`（管理UI）・`common/`（ヘッダー/フッター/認証/共通部品）。`pathPrefix:false` で名前は据え置き（`<UserTable>` 等そのまま） |
| `app/composables/useCurrentUser.ts` | ログインユーザーのプロフィールとロールを返す |
| `app/middleware/auth.global.ts` | ロールベースリダイレクト（`/admin` のみガード） |
| `server/api/auth/` | login / logout / reset-password（OTPコード送信）/ reset-password-otp（検証＋更新）/ update-password。app からは Supabase を直接呼ばず必ずここ経由 |
...
| `server/api/users/` | GET /me（自身）、GET 一覧・POST 招待（社員ID手入力）・PUT 更新（管理者のみ） |
| `server/utils/` | server 専用ロジック: `auth`（`serverUserId` 認証ゲート・`assertAdminRole` 管理者ガード）・`supabaseError`（`throwSupabaseError`）・`validate`（`parseBody`/`parseQuery`）・`year` |
...
| `supabase/seed.sql` | ローカル/E2E 用シード（trainee/mentor/admin のテストユーザー＋担当割り当て。`supabase db reset` で適用） |
| `e2e/` | Playwright E2E（`fixtures.ts` ＋ ログイン認証・日報CRUD・週次コメント。ローカル Supabase に対して実行） |
...
E2E は **Playwright**（`e2e/`）。`@nuxtjs/supabase` 越しの実 RLS を検証するため、ローカル Supabase スタック（`supabase start` → `supabase db reset` で `supabase/seed.sql` 適用）に対して実行する。Playwright の `*.spec.ts` は `e2e/**` 配下に置き、`vitest.config.ts` の exclude で Vitest から除外している（衝突回避）。CI では `.github/workflows/ci.yml`（test/build/gitleaks/audit）に加え `.github/workflows/e2e.yml` が**非ブロッキング**で実行。
