...
**フォーム（全項目を同時に表示）**
- メールアドレス入力フィールド ＋ 右にインラインの「送信」ボタン（認証コード(OTP)をメール送信。桁数は `otp_length` 設定依存。リンクは使わない。フォーム送信とは別アクション）
...
- 週間日報リスト（月〜金、曜日付き）
- 週次コメントエリア
- 週次サマリーエリア: メンター・OJT・管理者は選択中新人の mood 推移グラフ（直近8週・日次）＋AI 週次サマリー（観察視点）。新人本人は自分の振り返りサマリー（mood グラフなし）。サマリーは生成/再生成ボタン・鮮度バッジ・「参考情報」注記つき。生成は SSE ストリーミングで本文が逐次表示される
- 日報入力・編集モーダル（新人のみ操作可）。「ヒントを見る（AI）」で振り返りの深掘り質問＋短評を表示（本文の代筆・挿入はしない）
