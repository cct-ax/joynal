---
description: 6テーブルの ER 図・カラム定義・制約・インデックス（SQL 付き）。
---

# DBテーブル設計書

このページで分かること: 全テーブルの ER 図、各テーブルのカラム・制約・インデックス、生成 SQL。

> 関連: [RLS 設計](./rls-design.md) ・ [アーキテクチャ](./architecture.md) ・ [用語集](./glossary.md)
